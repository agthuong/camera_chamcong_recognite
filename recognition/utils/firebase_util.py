...
logger = setup_logger(__name__, 'firebase.log')
...
def initialize_firebase():
    ...
    try:
        ...
        logger.info(f"[FIREBASE] Đang khởi tạo Firebase với file xác thực: {service_account_path}")
    ...
    try:
        ...
        if camera_name:
            try:
                ...
                logger.error(f"[FIREBASE] Lỗi khi cập nhật camera: {e}")
        ...
        try:
            ...
            logger.error(f"[FIREBASE] Lỗi khi lấy thông tin vai trò: {e}")
        ...
        if user_role == 'worker' and supervisor_email:
            try:
                ...
                logger.info(f"[FIREBASE] Đã cập nhật worker ID {worker_id_for_list} với thông tin chi tiết vào danh sách của supervisor {supervisor_email}")
            except Exception as e:
                logger.error(f"[FIREBASE ERROR] Lỗi khi cập nhật list_worker cho {supervisor_email}: {e}")
        return True
