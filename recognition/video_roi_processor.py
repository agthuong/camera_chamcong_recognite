...
def test_roi_recognition(video_source, roi, show_window=False):
    """
    Test nhận diện với ROI đã chọn.
    Args:
        video_source: Nguồn video (0 cho webcam, đường dẫn file, hoặc RTSP URL)
        roi: Tuple (x, y, w, h) của vùng ROI
        show_window: Hiển thị cửa sổ OpenCV hay không
    Returns:
        Dictionary chứa tên người được nhận diện và số frame nhận diện được
    """
    print(f"[Test ROI] Bắt đầu test với nguồn: {video_source}, ROI: {roi}")
    
    # Khởi tạo video capture
    cap = cv2.VideoCapture(video_source)
    if not cap.isOpened():
        print(f"[Test ROI] Không thể mở video source: {video_source}")
        return None

    # Khởi tạo face detector và aligner
    detector = dlib.get_frontal_face_detector()
    ...
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
                print("[Test ROI] Không thể đọc frame từ video")
                break

            frame_count += 1
            print(f"[Test ROI] Đang xử lý frame {frame_count}")

            # Lấy vùng ROI từ frame
            x, y, w, h = roi
            roi_frame = frame[y:y+h, x:x+w]
            
            if roi_frame.size == 0:
                print(f"[Test ROI] ROI trống tại frame {frame_count}")
                continue
            ...
            # Phát hiện khuôn mặt trong ROI
            faces = detector(roi_gray, 0)
            print(f"[Test ROI] Tìm thấy {len(faces)} khuôn mặt trong frame {frame_count}")
            ...
            for face in faces:
                try:
                    ...
                    if not face_encodings:
                        print(f"[Test ROI] Không thể trích xuất encoding từ khuôn mặt ở frame {frame_count}")
                        continue

                    # Dự đoán người
                    pred_idx, pred_prob = predict(face_encodings[0])
                    if pred_idx[0] != -1:  # Nếu nhận diện được
                        name = le.classes_[pred_idx[0]]
                        prob = pred_prob[0]
                        print(f"[Test ROI] Nhận diện được {name} với xác suất {prob:.2f} tại frame {frame_count}")
                    ...
                    # Vẽ bounding box và tên (nếu show_window=True)
                    if show_window:
                        (fx, fy, fw, fh) = face_utils.rect_to_bb(face)
                        orig_x = x + fx
                        orig_y = y + fy
                        ...
                        # Vẽ bounding box
                        cv2.rectangle(frame, (orig_x, orig_y), (orig_x + fw, orig_y + fh), (0, 255, 0), 2)
                        
                        # Hiển thị tên và xác suất
                        if pred_idx[0] != -1:
                            text = f"{name} ({prob:.2f})"
                    ...
                    print(f"[Test ROI] Lỗi khi xử lý khuôn mặt: {e}")
                    continue

            # Hiển thị frame (nếu show_window=True)
            if show_window:
                # Vẽ ROI
                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
                
                cv2.imshow("Test ROI", frame)
                key = cv2.waitKey(1) & 0xFF
                if key == ord('q'):  # Thoát nếu nhấn 'q'
                    break

    except Exception as e:
        print(f"[Test ROI] Lỗi trong quá trình test: {e}")
    finally:
        cap.release()
        if show_window:
            cv2.destroyAllWindows()

    print(f"[Test ROI] Kết thúc test. Tổng số frame đã xử lý: {frame_count}")
